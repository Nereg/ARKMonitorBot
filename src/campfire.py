...
class Campfire(commands.Cog):
    ...
    def calculate(self, meatCount:int, meatType:Meat, campCount:int = 1):
        ...
        self.cookTime = ((meatCount * meatType.cookTime()) / campCount) / self.rate # in seconds (maybe)
    ...
    async def campfire(self,ctx, ammount: int = None, camps: int = 1):
        ...
        if (ammount == None):
            ...
            emb.set_footer(text=f'Requested by {ctx.author.name} • Bot {self.cfg.version} • GPLv3 ',icon_url=ctx.author.avatar_url)
            emb.set_thumbnail(url='https://static.wikia.nocookie.net/arksurvivalevolved_gamepedia/images/0/01/Campfire.png')
            ...
            emb.add_field(name='Cook time of any fish or meat:',value='20 seconds',inline=False)
        ...
        if (camps == 1):
            ...
            emb.add_field(name='Thatch (per camp):',value=self.thatchPerCamp,inline=False)
