...
class Automessage(commands.Cog):
    ...
    # check for : send,edit messages , use external emojis, ...
    async def checkPermissions(self, channel, channel_id=0):
        # people mess those up every time and blame it on me ! (and oh wait it can't fail...)
        return True
        if (channel_id != 0):
        ...
        try:
            ...
            if (agrs[0] != 'delete'):
                ...
                if (messages.__len__() <= 0):
                    embed = discord.Embed(
                        title='You have no automessage for that server!')
                    embed.set_footer(
            ...
            for message in messages:
                ...
                i += 1
            if (messages.__len__() <= 0):
                embed.add_field(name='Oops seems like you have no auto messages!',
                                value=f'But you can add them via {ctx.prefix}automessage #channel_you_want')
            embed.set_footer(
                text=f'Requested by {ctx.author.name} • Bot {self.cfg.version} • GPLv3 ', icon_url=ctx.author.avatar_url)
            await ctx.send(embed=embed)
            return
