...
async def fixIp(ip: str):
    ...
    async with aiohttp.request(
        "GET",
        f"http://api.steampowered.com/ISteamApps/GetServersAtAddress/v0001?addr={ip}",
        headers=HEADERS,
    ) as resp:
        ...
        if (
            bool(response["response"]["success"])
            and response["response"]["servers"].__len__() > 0
        ):
            ...
            for server in response["response"]["servers"]:
                ...
                ):  # if game port matches of that in the response
...
async def AddServer(ip, ctx):
    ...
    await makeAsyncRequest(
        "INSERT INTO servers(Ip,ServerObj,PlayersObj,Port) VALUES (%s,%s,%s,%s)",
        [ip, server.toJSON(), playersList.toJSON(), splitted[1]],
    )
    # search it's id
...
async def get_prefix(bot, message):
    ...
    data = await makeAsyncRequest(
        "SELECT * FROM settings WHERE GuildId=%s", (int(guildId),)
    )
...
async def getAlias(serverId, guildId, serverIp=""):
    if serverIp != "":
        serverId = await makeAsyncRequest(
            "SELECT Id FROM servers WHERE Ip=%s", (serverIp,)
        )
...
async def deleteServer(serverIp, Id=-1):
    ...
    automessages = await makeAsyncRequest(
        "SELECT * FROM automessages WHERE ServerId=%s", (serverId,)
    )
    ...
    for setting in settings:
        ...
        if serverId in ids:
            ids.remove(serverId)
            await makeAsyncRequest(
                "UPDATE settings SET ServersId=%s WHERE Id=%s",
                (
                    json.dumps(ids),
                    setting[0],
                ),
    ...
    if automessages.__len__() > 0:
        for message in automessages:
            await makeAsyncRequest(
                "DELETE FROM automessages WHERE Id=%s", (message[0],)
            )
        ...
        if newLine:
            ...
            for chunk in chunks:
                if chunk.__len__() >= 2000:  # oh shit
                    ...
                    testMegachunk = (
                        megachunk + chunk
                    )  # we will add it to iur mega chunk
                    if (
                        testMegachunk.__len__() >= 2000
                    ):  # if our test super chunk is over 2k
