...
class ARKServer(JSON):
    ...
    def __init__(self, ip):
        ...
        """
        self.ip = ip
        self.address, self.port = ip.split(":")  # split address to ip and port
    ...
    async def AGetInfo(self):
        ...
        """
        try:
            server = a2s.ainfo((self.address, self.port))  # create two corotines
            data = a2s.arules((self.address, self.port))
            server, data = await asyncio.gather(*[server, data])  # run them in parallel
        ...
        self.map = discord.utils.escape_mentions(
            server.map_name
        )  # get name of server's map
        self.password = (
            server.password_protected
        )  # get if server is protected by password
        try:
            self.PVE = bool(int(data["SESSIONISPVE_i"]))  # get if server is PVE
        except KeyError as e:
            raise ARKServerError(
                "6: Not an ARK Server!", e
            )  # it is not an ARK server !
        try:
            self.clusterName = discord.utils.escape_mentions(
                data["ClusterId_s"]
            )  # cluster name
        except KeyError:
        ...
        if "MOD0_s" in data:  # if we have any mods on server
            ...
            while end:  # while not end
                if f"MOD{i}_s" in data:  # if we have i-th mod in data
                    self.mods.append(
                        data[f"MOD{i}_s"].split(":")[0]
                    )  # append it's id to the list
                    i += 1  # increase i
                else:
...
class PlayersList(JSON):
    ...
    async def AgetPlayersList(self):
        ...
        for player in players:  # for each player in data
            ...
            )  # construct player class and append it to our results
        self.list = result
        return self
...
class Translation:  # not used just a param to some classes/functions
    def load_file(self, lang, name="translations"):
        try:
            ...
            raise Exception(
                "Tarnslation file `{}` not found in `{}`!".format(lang, path)
            )
