...
class Campfire(commands.Cog):
    ...
    def calculate(self, meatCount: int, meatType: Meat, campCount: int = 1):
        ...
        self.cookTime = ((meatCount * meatType.cookTime()) /
                         campCount) / self.rate  # in seconds (maybe)
        ...
        self.cookTimeSec = str(
            int(self.cookTime - (int(self.cookTimeMin) * 60))).zfill(2)
    ...
    async def campfire(self, ctx, ammount: int = None, camps: int = 1):
        ...
        if (ammount == None):
            ...
            emb.add_field(name='Thatch burn time:',
                          value='7.5 seconds', inline=False)
            emb.add_field(name='Wood burn time:',
                          value='30 seconds', inline=False)
            emb.add_field(name='Cook time of any fish or meat:',
                          value='20 seconds', inline=False)
            emb.add_field(name='Cook time of mutton:',
                          value='1 minute', inline=False)
            await ctx.send(embed=emb)
            return
        if (camps <= 0):
            await self.sendEmbed(ctx, 'Oops!', 'You can`t use 0 or negative numbers for `camps` parameter!', "Just don't. You can't cook something in 0 camps, can you?")
            return
        ...
        if (camps == 1):
            ...
            emb.add_field(name='Thatch:',
                          value=self.thatchPerCamp, inline=True)
            ...
            emb.add_field(
                name='Time:', value=f'{self.cookTimeHor}:{self.cookTimeMin}:{self.cookTimeSec}', inline=False)
            emb.add_field(name='Thatch (per camp):',
                          value=self.thatchPerCamp, inline=False)
            emb.add_field(name='Wood (per camp):',
                          value=self.woodPerCamp, inline=False)
            emb.add_field(name='Total thatch:',
                          value=self.totalThatch, inline=False)
            emb.add_field(name='Total wood:',
                          value=self.totalWood, inline=False)
            emb.set_thumbnail(
                url='https://static.wikia.nocookie.net/arksurvivalevolved_gamepedia/images/0/01/Campfire.png')
            await ctx.send(embed=emb)
            return
