...
class SlashCommandsHelper(commands.Cog):
    ...
    async def listAllCommands(self,ctx,GuildId:int = None):
        ...
        resp = await self.httpSession.get(f'https://discord.com/api/v8/applications/{self.appId}/commands',
        ...
        resp = await self.httpSession.get(f'https://discord.com/api/v8/applications/{self.appId}/guilds/{GuildId}/commands',
                                    headers=self.authheader)
    ...
    @commands.command()
    async def addGuildCommand(self, ctx, Name:str, Description:str, Type = 1, guildId = None, *,RawJson=None):
        if (not guildId):
            guildId = ctx.guild.id
