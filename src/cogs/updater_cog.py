...
class UpdateResult(c.JSON):
    def __init__(
        self,
        result: bool,
        serverObj: c.ARKServer,
        playersObj: c.PlayersList,
        serverRecord,
        reason: c.ARKServerError = None,
    ):
        self.result = result  # true - update is successful, false - not successful
        self.serverObj = (
            serverObj  # updated server object (can be None if unsuccessful)
        )
        self.playersObj = (
            playersObj  # updated players object (can be None if unsuccessful)
        )
        ...
        self.cachedServer = c.ARKServer.fromJSON(
            serverRecord[4]
        )  # make classes out of JSON
        self.cachedPlayers = c.PlayersList.fromJSON(serverRecord[5])
        self.Id = serverRecord[0]  # id of the server in DB
        self.moreInfo = json.loads(
            serverRecord[8]
        )  # decode JSON with more info from DB
...
class NeoUpdater(commands.Cog):
    def __init__(self, bot) -> None:
        ...
        self.plugins = (
            []
        )  # list of classes which would receive batches of update results
        self.servers = (
            None  # local cache of all servers from DB. Updated on every iteration
        )
        self.serversIds = (
    ...
    async def updateServer(self, serverRecord):
        ...
        try:
            ...
            return UpdateResult(
                False, None, None, serverRecord, e
            )  # return fail and reason
        ...
        return UpdateResult(
            True, results[0], results[1], serverRecord
        )  # else return success
    ...
    async def save(self, results):
        ...
        for result in results:
            ...
            if result.successful():
                # make request
                task = self.makeAsyncRequest(
                    "UPDATE servers SET LastOnline=1, OfflineTrys=0, ServerObj=%s, PlayersObj=%s, Info=%s, LastUpdated = CURRENT_TIMESTAMP WHERE Id=%s",
                    (
                        result.serverObj.toJSON(),
                        result.playersObj.toJSON(),
                        json.dumps(result.moreInfo),
                        result.Id,
                    ),
                )
                ...
                # make request
                task = self.makeAsyncRequest(
                    "UPDATE servers SET LastOnline=0, OfflineTrys=%s, Info=%s, LastUpdated = CURRENT_TIMESTAMP WHERE Id=%s",
                    (
                        result.serverRecord[7] + 1,
                        json.dumps(result.moreInfo),
                        result.Id,
                    ),
                )
    ...
    async def update(self):
        ...
        self.servers = await self.makeAsyncRequest(
            "SELECT * FROM servers"
        )  # update local cache
