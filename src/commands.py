...
class BulkCommands(commands.Cog):
    ...
    async def list(self, ctx):
        ...
        data = await makeAsyncRequest('SELECT * FROM settings WHERE GuildId=%s AND Type=%s',(GuildId,Type)) # get settings of current guild
...
{discord.utils.escape_mentions(servers)}
    ...
    async def info(self, ctx):
        ...
        embed.add_field(name='<:DB:739476980075986976> Servers in database',value=f'{count[0][0]}',inline=True)
